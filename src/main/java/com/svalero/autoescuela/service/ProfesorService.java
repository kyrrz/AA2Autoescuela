...
public class ProfesorService {
    ...
    public ProfesorDetailOutDto addV2(ProfesorInDto profesorDto, List<AutoescuelaDetailOutDto> autoescuelaDetailOutDtos) throws AutoescuelaNotFoundException {
        String especialidad = profesorDto.getEspecialidad();

        if (especialidad == null || !especialidad.matches("^(Teórico|Práctico)$")) {
            throw new ValidationException("La especialidad '" + profesorDto.getEspecialidad() + "' no es válida.");
        }
        Profesor p = modelMapper.map(profesorDto, Profesor.class);
